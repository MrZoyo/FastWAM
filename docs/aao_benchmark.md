...
- 因为这份数据的 row 0 是“当前帧相对上一帧”的 backward delta，AAO bridge
  ...
  一帧时间对齐并作为绝对目标下发。
- open door 的 profile 固定 `stride: 32`。这个值表示一个 32-action
  chunk 全部执行完再重新规划。旧的 `stride=8` 容易只执行窗口前段的近似静止
  action，反复丢掉后段靠近门把手的关键动作；除非专门排查 receding-horizon
  行为，不要覆盖成 8。
- open door 的 profile 还固定 `disable_arm_randomization: true`。batch
  benchmark 会把它转成
  `task.randomization.arm.base/eef.{x,y,z}=[0.0,0.0]`，避免 AAO reset 把初始
  base/home 推到训练数据分布外。只有专门测试 reset 鲁棒性时，才传
  `--enable-arm-randomization` 重新打开。
...
100Hz AAO update；默认 `stride=32`，用于执行完整预测窗口后再重新规划；默认
关闭 arm base/EEF reset randomization。显式传 `--stride` 会覆盖 profile
stride，显式传 `--enable-arm-randomization` 会重新打开 reset 随机化。
...
- `--stride`：每个模型 chunk 只执行前几个 action，然后重新取 observation 推理。
  open door profile 默认是 32，也就是执行完整 `action_horizon=32` 后再重新规划；
  不建议再使用旧的 8 作为 open-door 正式评测值。
...
- `--enable-arm-randomization`：诊断用。open-door profile 默认关闭 arm
  base/EEF reset randomization；传这个参数会重新启用 profile 原始随机化。
