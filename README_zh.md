...
默认 open-door 任务是 `open_door_airbot_play_back_gs`，也就是门把手在左侧的
back 版本；这和当前本地 real+sim open-door 训练/回放数据一致。只有需要测试
front/right-handle 对照场景时，才显式传 `--task open_door_airbot_play_gs`。

闭环 runner、benchmark、visual rollout 和 sweep 脚本都不会硬编码 checkpoint
路径。使用 `--model-client fastwam` 时，必须显式传入和当前 run 匹配的
`--fastwam-config`、`--checkpoint`、`--dataset-stats` 和 `--text-cache-dir`。
`runs/` 下的模型运行目录不会提交进 git，且 `dataset_stats.json` 必须和
checkpoint 的训练数据匹配。
...
对于当前本地 open-door LeRobot 数据，action 频率是 20Hz，AAO open door 的
`env.update_freq` 是 100Hz。因此默认闭环参数会把每个模型 action 重复 5 个
AAO update，并执行完整 32-action window 后再重新规划：
...
open-door 闭环默认会关闭 AAO 的 arm base/EEF reset randomization，使仿真初始
状态接近本地 LeRobot replay 数据使用的固定 base/home 分布。只有专门测试 reset
鲁棒性时，才显式传 `--enable-arm-randomization`。

batch benchmark 的 profile 配置在 `configs/aao_benchmark/`，当前预设
`open_door_airbot_play_back_gs`、`open_door_airbot_play_gs` 和
`cup_on_coaster_gs_airbot_p7`。benchmark 默认 profile 是
`open_door_airbot_play_back_gs`；也可以通过 `--profile-config <yaml>` 接入新的
测试 env。AAO `apply_pose_action()` 需要的是
...
对于 `action_horizon=32` 的 open-door checkpoint，除非专门排查
receding-horizon 行为，不要再使用旧的 `stride=8`。本地 replay 显示，第一个
明显靠近门把手的大动作可能出现在首个 32-action window 的后段；只执行前 8 个
action 就重新规划，会反复丢掉后段关键动作。open-door benchmark profile 已记录
`stride: 32`，batch benchmark 未显式传 `--stride` 时会使用这个 profile 值。这个
profile 还记录了 `disable_arm_randomization: true`，因此 open-door batch
benchmark 默认关闭 AAO arm base/EEF reset randomization；只有显式传
`--enable-arm-randomization` 才会重新打开。

两个预设 profile 的模型输出都按 7D EEF pose + gripper 处理；open door
checkpoint 的模型输入 state 是 6D arm joint + gripper，cup profile 的模型输入
state 是 8D joint + gripper。二者都不走 `joint_absolute` 控制，动作仍统一以
`cartesian_absolute` 下发。cup checkpoint/profile 只验证 `cup_on_coaster_gs_airbot_p7` 这个 AAO
